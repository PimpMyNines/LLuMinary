...
def list_configs(config_dir: str):
    ...
    for config in configs:
        ...
        if config["metadata"]:
            click.echo(f"  Metadata: {json.dumps(config['metadata'], indent=2)}")
...
def validate(config_path: str):
    ...
    try:
        ...
        click.echo(f"✗ Configuration is invalid: {str(e)}", err=True)
        sys.exit(1)
...
@click.option(
    "--model", default="claude-sonnet-3.5", help="Model to use for classification"
)
@click.option("--system-prompt", help="Optional system prompt")
def test(config_path: str, message: str, model: str, system_prompt: Optional[str]):
...
@click.option("--description", prompt=True, help="Description of the classifier")
def create(output_path: str, name: str, description: str):
    ...
    if click.confirm("\nAdd example classifications?", default=True):
        ...
        while True:
            user_input = click.prompt("User input", default="", show_default=False)
            if not user_input:
                break
            ...
            examples.append(
                {"user_input": user_input, "doc_str": doc_str, "selection": selection}
            )
