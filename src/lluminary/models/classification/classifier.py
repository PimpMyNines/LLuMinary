...
class Classifier:
    ...
    def classify(
        ...
        config = ClassificationConfig(
            categories=categories, examples=examples or [],
        ...
        response, usage = self.llm.generate(
            ...
            messages=messages,
            result_processing_function=lambda
                x: validate_classification_response(
                x, len(categories), max_options
            ),
        )

        # Convert numeric selections to category names
        selections = self._convert_to_category_names(response,
    ...
    def _load_default_system_prompt(self) -> str:
        ...
        prompt_path = os.path.join(base_dir, "prompts", "classification",
                                   "base.yaml")
