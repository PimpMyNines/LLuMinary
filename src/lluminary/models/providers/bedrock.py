...
class BedrockLLM(LLM):
    ...
    def _map_aws_error(self, error: Exception) -> Exception:
        ...
        if isinstance(error, ClientError):
            ...
            if error_code in [
                ...
                return LLMAuthenticationError(
                    ...
                    provider="BedrockLLM",
                    details={"error_code": error_code, "original_error": error_message},
                )
        ...
        if any(
            ...
            for term in ["credential", "access", "permission", "auth", "token", "key"]
        ):
            return LLMAuthenticationError(
            ...
            term in error_message for term in ["config", "param", "invalid", "model"]
        ):
            return LLMConfigurationError(
    ...
    def _raw_generate(
        ...
        try:
            ...
            try:
                ...
                usage_stats["read_cost"] = float(usage_stats["read_tokens"]) * float(
                    costs["read_token"]
                )
                usage_stats["write_cost"] = float(usage_stats["write_tokens"]) * float(
                    costs["write_token"]
                )
