...
class BedrockLLM(LLM):
    ...
    def _process_image(self, image_path: str) -> Image.Image:
        ...
        try:
            ...
            raise LLMContentError(f"Failed to process image {image_path}: {e!s}")
    ...
    def _get_image_bytes(self, image_path: str) -> bytes:
        ...
        try:
            ...
            raise LLMContentError(
                f"Failed to convert image {image_path} to bytes: {e!s}"
            )
    ...
    def _download_image_from_url(self, image_url: str) -> bytes:
        ...
        try:
            ...
            raise LLMContentError(
                f"Failed to download image from URL {image_url}: {e!s}"
            )
        except Exception as e:
            raise LLMContentError(
                f"Failed to process image from URL {image_url}: {e!s}"
            )
    ...
    def _format_messages_for_model(
        ...
        for msg in messages:
            ...
            bedrock_messages.append({"role": role, "content": content_parts})
