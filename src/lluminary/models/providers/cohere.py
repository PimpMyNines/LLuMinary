...
class CohereLLM(LLM):
            ...
            if self.supports_image_input() and message_type == "human":
                ...
                for img_path in message.get("image_paths", []):
                    try:
                        ...
                        print(
                            f"Warning: Failed to process image {img_path}: {e!s}")
                ...
                for img_url in message.get("image_urls", []):
                    try:
                        ...
                        print(
                            f"Warning: Failed to add image URL {img_url}: {e!s}")
    ...
    def _raw_generate(
        ...
        if system_prompt:
            formatted_messages.insert(0, {"role": "SYSTEM",
        ...
        request_data = {
            ...
            "message": formatted_messages[-1][
                "message"] if formatted_messages else "",
            "chat_history": (
    ...
    def rerank(
        ...
        try:
            ...
            if top_n is not None and hasattr(response.results[0], "document"):
                ranked_documents = [result.document for result in
                                    response.results]
            else:
