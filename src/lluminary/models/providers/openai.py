...
class OpenAILLM(LLM):
        ...
        for message in messages:
            ...
            for image_path in message.get("image_paths", []):
                try:
                    ...
                    raise Exception(
                        f"Failed to process image file {image_path}: {e!s}"
                    )
            ...
            for image_url in message.get("image_urls", []):
                try:
                    ...
                    raise Exception(
                        f"Failed to process image URL {image_url}: {e!s}"
                    )
    ...
    def _raw_generate(
        ...
        Raises:
            LLuMinaryError: With appropriate error type based on the issue
        """
        try:
            # Format messages for OpenAI's API
            try:
    ...
    def _map_openai_error(self, error: Exception) -> LLuMinaryError:
        """
        Map OpenAI-specific errors to standardized LLuMinary error types.

        This method analyzes an exception from the OpenAI API and converts it
        to the appropriate LLuMinary exception type with relevant context.

        Args:
            error: The original OpenAI exception

        Returns:
            LLuMinaryError: The mapped exception with detailed context
        """
        import openai
