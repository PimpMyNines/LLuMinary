...
def validate_tool(tool: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        # Validate docstring
        if not tool.__doc__:
            raise ToolValidationError(f"Tool '{tool.__name__}' must have a docstring")

        # Get and validate signature
        try:
            sig = inspect.signature(tool)
        except ValueError as e:
            raise ToolValidationError(f"Could not inspect tool '{tool.__name__}': {e}")
...
def _validate_type_structure(value: Any, expected_type: Type) -> bool:
    ...
    if origin is tuple or origin is Tuple:
        ...
        return all(_validate_type_structure(val, typ) for val, typ in zip(value, args))
