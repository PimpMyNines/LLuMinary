...
from typing import Any, Optional, Optional, Dict, List, Optional, Set
...
def validate_max_tokens(max_tokens: int,
    ...
    for i, tool in enumerate(tools):
        ...
        if missing_keys:
            raise LLMValidationError(
                ...
                details={"index": i, "missing_keys": list(missing_keys),
                         "tool": tool},
            )
        ...
        if not isinstance(tool["name"], str):
            raise LLMValidationError(
                ...
                details={"index": i,
                         "provided_type": type(tool["name"]).__name__},
            )
        ...
        if "input_schema" in tool and not isinstance(tool["input_schema"],
                                                     dict):
            raise LLMValidationError(
        ...
        if not isinstance(description, str):
            raise LLMValidationError(
                ...
                details={"category": name,
                         "provided_type": type(description).__name__},
            )
