...
from src.llmhandler.models.router import get_llm_from_model, MODEL_MAPPINGS
...
class TestDynamicModelSelection:
    ...
    def test_model_fallback_chain(self, handler):
        ...
        assert successful_model in fallback_models, "Fallback model should be one of the specified models"
    ...
    def test_capability_based_routing(self):
        ...
        for model_name in test_models:
            try:
                ...
                supports_functions = (
                    ...
                    (hasattr(llm, "supports_function_calling") and llm.supports_function_calling()) or
                    # Or check for specific model names known to support function calling
                    any(substr in model_name for substr in ["gpt-", "claude-", "gemini-"])
                )
        ...
        def get_capable_model(required_capabilities):
            ...
            for capability, models in required_capabilities.items():
                ...
                if capability == "function_calling" and models and not function_capable_models:
    ...
    def test_cost_optimized_selection(self, handler):
        ...
        for model_name in MODEL_MAPPINGS.keys():
            try:
                ...
                if hasattr(llm, "COST_PER_MODEL") and model_name in llm.COST_PER_MODEL:
                    ...
                    avg_cost = (read_cost + write_cost) / 2 if (read_cost or write_cost) else 0
                    
                    if avg_cost > 0:
                        model_costs[model_name] = avg_cost
                        model_list.append(model_name)
                        print(f"Model {model_name}: avg cost = ${avg_cost:.6f} per token")
        ...
        try:
            ...
            if more_expensive_model != cheapest_model:
                ...
                print(f"Tokens: {expensive_usage['total_tokens']}, Cost: ${expensive_usage['total_cost']:.6f}")
                
                # Verify cost difference
                if expensive_usage['total_cost'] > usage['total_cost']:
                    print("✅ Confirmed cost difference between models")
                    print(f"Cost savings: ${expensive_usage['total_cost'] - usage['total_cost']:.6f}")
                else:
                    print("⚠️ Expected cost difference not observed")
            
        except Exception as e:
            print(f"Error testing cost-optimized selection: {str(e)}")
            pytest.skip(f"Could not test with cheapest model: {str(e)}")
    ...
    def test_content_based_routing(self, handler):
        ...
        for complexity, prompt in prompts.items():
            ...
            try:
                ...
                print(f"Error using selected model {model_name}: {str(e)}")
                continue
        ...
        assert len(selected_models) > 0, "Should have selected models for at least one complexity level"