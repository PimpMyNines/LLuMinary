...
class TestOptionalParameters:
    ...
    def test_temperature_parameter(self):
        ...
        for model_name in test_models:
            ...
            try:
                ...
                low_temp_similarity = simple_similarity(low_temp_responses[0], low_temp_responses[1])
                high_temp_similarity = simple_similarity(high_temp_responses[0], high_temp_responses[1])
                
                print(f"\nSimilarity between low temperature responses: {low_temp_similarity:.2f}")
                print(f"Similarity between high temperature responses: {high_temp_similarity:.2f}")
                ...
                if low_temp_similarity >= high_temp_similarity:
                    print("✅ Low temperature responses more similar than high temperature (expected)")
                else:
                    print("⚠️ High temperature responses more similar than low temperature (unexpected)")
    ...
    def test_openai_specific_parameters(self):
        ...
        try:
            ...
            print(f"Word variety with high frequency penalty: {high_penalty_variety:.3f}")
            ...
            if high_penalty_variety > no_penalty_variety:
                ...
                print("⚠️ High frequency penalty did not increase word variety (unexpected)")
            
        except Exception as e:
            print(f"Error testing OpenAI parameters: {str(e)}")
            pytest.skip(f"Could not test OpenAI-specific parameters: {str(e)}")
    ...
    def test_anthropic_specific_parameters(self):
        ...
        try:
            ...
            creative_prompt = "Generate a creative name for a space exploration company."
            ...
            thinking_supported = (
                hasattr(llm, "thinking_models") and 
                llm.model_name in getattr(llm, "thinking_models", [])
            )
            ...
            if thinking_supported:
                ...
                print(f"Tokens: {usage_no_thinking['total_tokens']}, Cost: ${usage_no_thinking['total_cost']:.6f}")
                ...
                print(f"Tokens: {usage_with_thinking['total_tokens']}, Cost: ${usage_with_thinking['total_cost']:.6f}")
                
                # Check for "180 miles" in both responses as basic verification
                has_answer_no_thinking = "180" in response_no_thinking or "180 miles" in response_no_thinking.lower()
                has_answer_with_thinking = "180" in response_with_thinking or "180 miles" in response_with_thinking.lower()
                
                print(f"\nCorrect answer without thinking budget: {has_answer_no_thinking}")
                print(f"Correct answer with thinking budget: {has_answer_with_thinking}")
                
            else:
                print(f"Model {model_name} does not support thinking budget, skipping...")
            
        except Exception as e:
            print(f"Error testing Anthropic parameters: {str(e)}")
            pytest.skip(f"Could not test Anthropic-specific parameters: {str(e)}")
