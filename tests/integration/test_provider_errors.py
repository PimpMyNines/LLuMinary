...
from src.llmhandler.exceptions import LLMProviderError
...
class TestProviderErrorTypes:
        ...
        for model_name in test_models:
            ...
            try:
                ...
                try:
                    ...
                    print(f"✓ Token limit error correctly raised: {type(e).__name__}: {str(e)}")
                ...
                try:
                    ...
                    print(f"✓ Invalid message error correctly raised: {type(e).__name__}: {str(e)}")
                    if provider not in error_types_by_provider:
                        error_types_by_provider[provider] = {}
                ...
                try:
                    ...
                    print(f"✓ Invalid API key error correctly raised: {type(e).__name__}: {str(e)}")
                    if provider not in error_types_by_provider:
                        error_types_by_provider[provider] = {}
                    ...
                    if hasattr(llm, "api_key") and 'original_api_key' in locals():
                        ...
                        if hasattr(llm.client, "api_key") and 'original_client_key' in locals():
                            llm.client.api_key = original_client_key
                        else:
                            llm.client = original_client
            
            except Exception as e:
                print(f"Error setting up tests for {model_name}: {str(e)}")
        ...
        for model_name in test_models:
            ...
            try:
                ...
                try:
                    ...
                    print("Note: Content policy error not raised (model may have handled the request safely)")
                except Exception as e:
            ...
            for error_case, errors in error_types_by_case.items():
                if errors:
                    ...
                    if len(error_types) == 1:
                        print(f"  ✅ Consistent error mapping: All providers use {next(iter(error_types))}")
                    else:
                        print(f"  ⚠️ Inconsistent error mapping: {', '.join(error_types)}")
        else:
    ...
    def test_error_details_extraction(self):
        ...
        try:
            ...
            pytest.skip(f"Error testing error details extraction: {str(e)}")