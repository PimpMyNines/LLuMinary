...
class TestRateLimitingBehavior:
        ...
        try:
            ...
            if hasattr(llm, "client"):
                ...
                if model_name.startswith("gpt"):
                    ...
                    rate_limit_error = Exception("Rate limit error: 429 Too Many Requests")
                else:
                ...
                print(f"Generating with {model_name} (should retry after rate limit)...")
                ...
                pytest.skip(f"Model {model_name} doesn't have a client attribute to patch")
                
        except Exception as e:
            if original_client:
                llm.client = original_client
            pytest.skip(f"Error testing rate limit recovery: {str(e)}")
    ...
    def test_concurrent_request_throttling(self):
        ...
        try:
            ...
            rate_limits = [r for r in failures if "429" in str(r["error"]) or "rate" in str(r["error"]).lower()]
    ...
    def test_progressive_backoff_strategy(self):
        ...
        try:
            ...
            if hasattr(llm, "client"):
                ...
                # Run test with retry mechanism
                try:
                    print(f"Generating with {model_name} (should retry multiple times)...")
                    ...
                    if len(delays) >= 2:
                        ...
                        progressive = all(delays[i] >= delays[i-1] * 0.8 for i in range(1, len(delays)))
                        if progressive:
            ...
            if original_client:
                llm.client = original_client
            pytest.skip(f"Error testing progressive backoff: {str(e)}")