...
from src.llmhandler.tools.validators import json_serializable, validate_params, validate_return_type
...
class TestToolRegistryIntegration:
    ...
    def registry(self):
        ...
        def search_database(term: str, query: str) -> List[Dict[str, str]]:
            ...
            return [
                {"id": "1", "title": f"Result for {term}", "content": f"Information about {query}"},
                {"id": "2", "title": f"Another result for {term}", "content": f"More about {query}"}
            ]
    ...
    def test_tool_function_calling_integration(self):
        ...
        for model_name in test_models:
            ...
            try:
                ...
                weather_terms = ["weather", "temperature", "degrees", "sunny", "celsius", "fahrenheit"]
                found_terms = [term for term in weather_terms if term in response.lower()]
        ...
        if not successful_models:
            pytest.skip("Skipping test as no models were able to complete the function calling test")
