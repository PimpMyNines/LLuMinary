...
class TestProviderTemplateStreaming:
    ...
    def test_stream_generate_implementation(self, provider_instance):
        ...
        def mock_stream_generate(self, event_id, system_prompt, messages, max_tokens=1000, temp=0.0, top_k=200, tools=None, thinking_budget=None):
        ...
        with patch.object(provider_instance, "_format_messages_for_model") as mock_format:
    ...
    def test_stream_generate_with_error(self, provider_instance):
        ...
        def mock_stream_generate_with_error(self, event_id, system_prompt, messages, max_tokens=1000, temp=0.0, top_k=200, tools=None, thinking_budget=None):
        ...
        # Add the method to the instance
        provider_instance._stream_generate = mock_stream_generate_with_error.__get__(provider_instance)
        
        # Add error handling wrapper method
        def stream_generate_with_error_handler(self, event_id, system_prompt, messages, max_tokens=1000, temp=0.0, top_k=200, tools=None, thinking_budget=None):
            """Error handling wrapper for streaming."""
            try:
                for chunk, usage in self._stream_generate(event_id, system_prompt, messages, max_tokens, temp, top_k, tools, thinking_budget):
                    yield chunk, usage
            except Exception as e:
                # Convert to LLMMistake exception
                raise LLMMistake(
                    f"Error during streaming with {self.__class__.__name__}: {str(e)}",
        ...
        provider_instance.stream_generate_with_error_handler = stream_generate_with_error_handler.__get__(provider_instance)
        
        # Mock format_messages
        with patch.object(provider_instance, "_format_messages_for_model", return_value=[]):
            # Call stream_generate and expect exception
            chunks = []
            with pytest.raises(LLMMistake) as excinfo:
                for chunk, usage in provider_instance.stream_generate_with_error_handler(
                    event_id="test-event",
            ...
            assert "Simulated API error" in str(excinfo.value.details.get("original_error", ""))
    ...
    def test_stream_generate_with_tools(self, provider_instance):
        ...
        def mock_stream_with_tools(self, event_id, system_prompt, messages, max_tokens=1000, temp=0.0, top_k=200, tools=None, thinking_budget=None):
        ...
        # Add the method to the instance
        provider_instance._stream_generate = mock_stream_with_tools.__get__(provider_instance)
    ...
    def test_stream_generate_token_counting(self, provider_instance):
        ...
        def mock_stream_with_token_counting(self, event_id, system_prompt, messages, max_tokens=1000, temp=0.0, top_k=200, tools=None, thinking_budget=None):
        ...
        # Add the method to the instance
        provider_instance._stream_generate = mock_stream_with_token_counting.__get__(provider_instance)
