...
from src.llmhandler.exceptions import ThinkingError
...
def test_thinking_budget_generation(anthropic_llm):
    ...
    with patch("requests.post", return_value=mock_response) as mock_post:
        ...
        assert "Let me analyze this problem step by step" in thinking_message["thinking"]["thinking"]
...
def test_thinking_error_handling(anthropic_llm):
    ...
    mock_response.raise_for_status.side_effect = Exception("Thinking is not supported by this model")
    mock_response.json.return_value = {"error": {"message": "Thinking is not supported by this model"}}
