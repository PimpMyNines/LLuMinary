...
class TestCohereAuthentication:
    ...
    def test_auth_with_patched_env(self):
        """Simple test using environment variable."""
        # Create a new instance
        llm = CohereLLM("command")

        # Mock environment variable directly
        with patch.dict(os.environ, {"COHERE_API_KEY": "test-env-key"}, clear=False):
            # Call auth method
            llm.auth()

            # Verify the API key was set
            assert llm.api_key == "test-env-key"
