...
class TestGoogleLLMMessageFormatting:
    ...
    def test_format_human_message(self, mock_auth):
        ...
        with patch("google.genai.types.Content") as mock_content, \
             patch("google.genai.types.Part") as mock_part:
    ...
    def test_format_message_with_local_image(self, mock_auth):
        ...
        with patch.object(llm, "_process_image", return_value=mock_image) as mock_process:
            # Setup other mocks
            with patch("google.genai.types.Content") as mock_content, \
                 patch("google.genai.types.Part") as mock_part:
    ...
    def test_format_message_with_url_image(self, mock_auth):
        ...
        with patch.object(llm, "_process_image", return_value=mock_image) as mock_process:
            ...
            with patch("google.genai.types.Content") as mock_content, \
                ...
                mock_process.assert_called_with("https://example.com/image.jpg", is_url=True)
...
class TestGoogleLLMRawGeneration:
    ...
    def test_raw_generate_with_images(self, mock_auth):
        ...
        with patch.object(llm, "_format_messages_for_model", return_value=formatted_messages) as mock_format:
            ...
            mock_response.usage_metadata.prompt_token_count = 1000  # Higher due to images
...
class TestGoogleLLMStreaming:
    ...
    @pytest.mark.skip(reason="Requires google.generativeai which is not installed in the test environment")
    @patch.object(GoogleLLM, "auth")
