...
def test_raw_generate_with_tools_debug(mock_auth):
    ...
    with patch.object(llm, "_format_messages_for_model", return_value=formatted_messages) as mock_format:
        ...
        with patch("google.genai.types.GenerateContentConfig", return_value=config_mock) as mock_config_class:
            ...
            try:
                ...
                print(f"generate_content called: {models_mock.generate_content.call_count}")
                ...
                assert False, f"Test raised an exception: {str(e)}"
...
def test_stream_generate_basic_debug(mock_auth):
    ...
    try:
        ...
        with patch("google.generativeai.GenerativeModel") as mock_genai_model, \
             patch("google.genai.GenerativeModel") as mock_model_class, \
             patch("google.genai.types.content_types") as mock_content_types, \
             patch("google.genai.types.generation_types") as mock_generation_types, \
             patch.object(llm, "_format_messages_for_model", return_value=[MagicMock()]):
            
            # Print import mocks
            print(f"Mock imports set up: {mock_genai_model}, {mock_content_types}, {mock_generation_types}")
            ...
            try:
                ...
                print(f"stream_generate method exists: {hasattr(llm, 'stream_generate')}")
                
                # Print method content using inspect
                import inspect
                print(f"Stream generate code:\n{inspect.getsource(llm.stream_generate)}")
                
                # Print current imported modules
                import sys
                print(f"Current loaded modules related to google: {[m for m in sys.modules.keys() if 'google' in m]}")
                ...
                print(f"generate_content called: {mock_model.generate_content.call_count}")
                if mock_model.generate_content.call_count > 0:
                    print(f"generate_content args: {mock_model.generate_content.call_args}")
                
                # Success if we reached here
                assert True
                
            except Exception as e:
                print(f"\nEXCEPTION in stream_generate: {str(e)}")
                import traceback
                traceback.print_exc()
