...
def test_map_google_error():
    ...
    test_cases = [
        ...
        {"message": "Invalid authentication credentials", "expected_type": AuthenticationError},
        ...
        {"message": "503 Service Temporarily Unavailable", "expected_type": ServiceUnavailableError},
        ...
        {"message": "Safety settings blocked the request", "expected_type": ContentError},
        {"message": "Harmful content detected", "expected_type": ContentError},
        {"message": "Content moderation flagged the input", "expected_type": ContentError},
    ...
    for case in test_cases:
        ...
        assert isinstance(mapped_error, case["expected_type"]), \
...
def test_auth_failures():
    ...
    with patch("src.llmhandler.utils.get_secret", side_effect=Exception("Secret not found")):
        with patch("os.environ", {}):  # Ensure no env vars
            ...
            assert "secret not found" in str(error.details.get("original_error", "")).lower()
    
    # Test with client initialization failure
    with patch("src.llmhandler.utils.get_secret", return_value={"api_key": "test_key"}):
        with patch("google.genai.Client", side_effect=Exception("Client initialization failed")):
            ...
            assert error.provider == "GoogleLLM"
            assert "client initialization failed" in str(error.details.get("original_error", "")).lower()
...
def test_image_processing_errors(mock_path_exists, mock_image_open, google_llm):
    ...
    with patch("requests.get") as mock_get:
        ...
        with pytest.raises(LLMMistake) as exc_info:
            google_llm._process_image("https://example.com/server-error.jpg", is_url=True)
        ...
        with pytest.raises(LLMMistake) as exc_info:
            google_llm._process_image("https://example.com/connection-error.jpg", is_url=True)
        ...
        with patch("io.BytesIO") as mock_bytesio:
            ...
            with pytest.raises(ContentError) as exc_info:
                google_llm._process_image("https://example.com/invalid-format.jpg", is_url=True)
            
            error = exc_info.value
            assert "Invalid or unsupported image format" in str(error)
            assert "Cannot identify image" in str(error.details.get("original_error", ""))
...
def test_raw_generate_error_handling(google_llm, mock_google_client):
    ...
    with patch.object(google_llm, "auth", side_effect=AuthenticationError("Auth failed", "GoogleLLM")):
    ...
    with patch.object(
        google_llm, "_format_messages_for_model", 
    ...
    client_instance.models.generate_content.side_effect = Exception("Rate limit exceeded")
    ...
    client_instance.models.generate_content.side_effect = Exception("Service unavailable")
    ...
    client_instance.models.generate_content.side_effect = Exception("Invalid model configuration")
...
def test_stream_generate_error_handling(mock_generative_model, google_llm):
    ...
    with patch.object(google_llm, "auth", side_effect=AuthenticationError("Auth failed", "GoogleLLM")):
        ...
        with pytest.raises(AuthenticationError):
            list(google_llm.stream_generate(
                event_id="test", system_prompt="", messages=messages
            ))
    ...
    with patch.object(google_llm, "_map_google_error", return_value=ServiceUnavailableError("Test mapped error", "GoogleLLM")) as mock_map:
