...
class TestGoogleLLMStreaming:
    ...
    def test_stream_generate_basic(self, mock_auth):
        ...
        try:
            ...
            mocks['google.genai.types.content_types'].Content.return_value = mock_system_content
            mocks['google.genai.types.content_types'].Part.from_text.return_value = mock_system_part
            
            # Mock _format_messages_for_model
            with patch.object(llm, "_format_messages_for_model", return_value=[MagicMock()]):
    ...
    def test_stream_generate_error_handling(self, mock_auth):
        ...
        try:
            ...
            mocks['google.generativeai'].GenerativeModel.side_effect = Exception("API Error")
            
            # Call stream_generate and expect exception
            with pytest.raises(Exception) as exc_info:
                list(llm.stream_generate(
                    event_id="test_event",
                    system_prompt="You are a helpful assistant",
                    messages=messages
                ))
