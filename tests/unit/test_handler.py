...
def test_tool_execution():
    ...
    tool_dict = {
        ...
        "parameters": {
            "type": "object",
            "properties": {"x": {"type": "integer", "description": "Number to double"}},
            "required": ["x"],
        },
    }
    print(f"Created tool_dict: {tool_dict}")
...
def test_classification():
    ...
    print("Created test message and categories")

    # Create a MagicMock for the classify method
    mock_classify_method = MagicMock(
        return_value=(
            {"question": 0.9, "statement": 0.1, "command": 0.0},
            {"read_tokens": 10, "write_tokens": 5, "total_cost": 0.02},
        )
    )
...
def test_cost_estimation():
    ...
    mock_get_model_costs_method = MagicMock(
        return_value={
            "read_token": 0.001,
            "write_token": 0.002,
            "image_cost": 0.01,
...
def test_context_window():
    ...
    with patch.object(LLMHandler, "get_provider", return_value=mock_llm):
        ...
        try:
            ...
            print(
                f"Fits: {fits}, Tokens used: {tokens_used}, Tokens remaining: {tokens_remaining}"
            )
            assert fits is True
...
def test_provider_specific_features():
    ...
    with patch.object(LLMHandler, "get_provider", return_value=mock_llm):
        ...
        try:
            ...
            response = handler.generate(messages=[test_message], tools=[tool_dict])
        ...
        try:
            response = handler.generate(messages=[test_message], temperature=0.7)
