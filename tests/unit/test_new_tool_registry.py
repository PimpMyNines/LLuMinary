...
from src.llmhandler.tools.registry import ToolMetadata, ToolRegistry, ToolValidationError
from src.llmhandler.tools.validators import json_serializable, validate_params, validate_return_type, validate_tool
...
def test_wrap_tool():
    ...
    print(f"Tool stats after success: {registry._tools[tool_name].success_count} successes, {registry._tools[tool_name].failure_count} failures")
    ...
    print(f"Tool stats after failure: {registry._tools[tool_name].success_count} successes, {registry._tools[tool_name].failure_count} failures")
    assert registry._tools[tool_name].success_count == 1
...
def test_register_tool():
    ...
    registry.register_tool(
        tool_with_type_hints, name="custom_tool", 
...
def test_list_tools():
    ...
    required_fields = {
        ...
        "success_count", "failure_count", "average_execution_time"
    }
