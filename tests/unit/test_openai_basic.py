...
def test_provider_properties(mock_openai_class):
    ...
    try:
        ...
        print(
            f"Checking embedding_costs: {llm.embedding_costs if hasattr(llm, 'embedding_costs') else 'Not found'}"
        )
        assert len(llm.embedding_costs) > 0
        assert llm.embedding_costs["text-embedding-3-small"] > 0
    except Exception as e:
        print(f"Exception in test_provider_properties: {type(e).__name__}: {e!s}")
        import traceback
