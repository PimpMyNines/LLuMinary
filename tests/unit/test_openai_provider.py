...
from src.llmhandler.exceptions import LLMMistake, AuthenticationError
...
def test_supported_model_lists(openai_llm):
    ...
    assert all(model in openai_llm.SUPPORTED_MODELS for model in openai_llm.THINKING_MODELS)
...
def test_openai_initialization(openai_llm):
    ...
    with patch.object(OpenAILLM, "auth"):
        llm = OpenAILLM("gpt-4o", api_key="test-key", api_base="https://custom-openai.example.com")
...
@patch("src.llmhandler.utils.get_secret")
...
def test_tool_formatting(openai_llm):
    ...
    assert formatted_dict_tool[0]["function"]["description"] == "A test tool defined as a dictionary"
    ...
    assert formatted_dict_tool[0]["function"]["parameters"]["additionalProperties"] is False
...
def test_image_support(openai_llm):
    ...
    with patch.object(openai_llm, "_encode_image_url", return_value="base64_data"):
        ...
        image_url_item = next(item for item in formatted[0]["content"] if item.get("type") == "image_url")
        assert image_url_item["image_url"]["url"] == "data:image/jpeg;base64,base64_data"
...
def test_encode_image_url(mock_requests_get, openai_llm):
    ...
    with patch("PIL.Image.open", return_value=mock_img), \
         patch.object(mock_img, "save", MagicMock()), \
        ...
        mock_requests_get.assert_called_once_with("http://example.com/image.jpg", timeout=10)
...
def test_encode_image_url_with_transparency(mock_requests_get, openai_llm):
    ...
    mock_img.size = (100, 100)
    mock_img.split = MagicMock(return_value=[MagicMock(), MagicMock(), MagicMock(), MagicMock()])
    ...
    with patch("PIL.Image.open", return_value=mock_img), \
         patch("PIL.Image.new", return_value=mock_background), \
         patch.object(mock_background, "paste", MagicMock()), \
         patch.object(mock_background, "save", MagicMock()), \
         patch("io.BytesIO", return_value=mock_output):
...
def test_image_message_formatting_combined(openai_llm):
    ...
    with patch.object(openai_llm, "_encode_image", return_value="local_base64"), \
        ...
        image_parts = [item for item in formatted[0]["content"] if item.get("type") == "image_url"]
        assert len(image_parts) == 4
        
        # Verify local and URL images are formatted correctly
        local_images = [item for item in image_parts if "local_base64" in item["image_url"]["url"]]
        url_images = [item for item in image_parts if "url_base64" in item["image_url"]["url"]]
...
def test_error_handling(openai_llm):
    ...
    error_cases = [
        ...
        {
            "exception": Exception("This model's maximum context length is 16385 tokens"),
...
def test_thinking_models(openai_llm):
    ...
    with patch.object(openai_llm, "client") as mock_client, patch.object(
        ...
        # Reset mock for o1 test
        mock_client.chat.completions.create.reset_mock()
        
        # Test reasoning model with enhanced reasoning
        with patch.object(openai_llm, "model_name", "o1"):
        ...
        with patch.object(openai_llm, "model_name", "o1"):
            ...
            # Verify reasoning_effort was set to high and tools were formatted
            call_args = mock_client.chat.completions.create.call_args[1]
            assert "reasoning_effort" in call_args
            assert call_args["reasoning_effort"] == "high"
...
def test_reranking_cost_calculation(openai_llm):
    ...
    with patch.object(openai_llm, "client") as mock_client:
        ...
        assert abs(usage["total_cost"] - expected_cost) < 0.000001  # Account for floating point precision
...
def test_streaming(openai_llm):
    ...
    mock_chunk_final.choices = [MagicMock(delta=MagicMock(content=None), finish_reason="stop")]
...
def test_streaming_with_tools(openai_llm):
    ...
    # Final chunk 
    mock_chunk_final = MagicMock()
    mock_chunk_final.choices = [MagicMock(delta=MagicMock(content=None), finish_reason="tool_calls")]
    mock_chunk_final.usage = MagicMock(prompt_tokens=15, completion_tokens=10, total_tokens=25)
