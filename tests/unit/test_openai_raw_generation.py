...
def test_basic_raw_generate(openai_llm):
    ...
    messages = [
        {"message_type": "human", "message": "Tell me a joke."}
    ]
    
    # Create mock response
    mock_response = MagicMock()
    mock_response.choices = [MagicMock(message=MagicMock(content="Why did the chicken cross the road?"))]
    ...
    assert len(call_args["messages"]) == 2  # One for system prompt, one for user message
...
def test_raw_generate_without_tools(openai_llm):
    ...
    messages = [
        {"message_type": "human", "message": "What's 2+2?"}
    ]
...
def test_raw_generate_with_reasoning_model(openai_llm):
    ...
    system_prompt = "You are a helpful assistant."
    messages = [
        {"message_type": "human", "message": "Solve this complex problem."}
    ]
    
    # Create mock response
    mock_response = MagicMock()
    mock_response.choices = [MagicMock(message=MagicMock(content="Detailed step-by-step solution"))]
...
def test_raw_generate_error_handling(openai_llm):
    ...
    # Mock API error
    openai_llm.client.chat.completions.create.side_effect = Exception("API error: rate limit exceeded")
