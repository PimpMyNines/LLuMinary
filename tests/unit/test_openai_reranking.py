...
def openai_llm():
    ...
    with patch.object(OpenAILLM, "auth") as mock_auth, \
         ...
         patch.object(OpenAILLM, "SUPPORTED_MODELS", ["gpt-4o", "text-embedding-3-small"]), \
         patch.object(OpenAILLM, "RERANKING_MODELS", ["text-embedding-3-small"]):
...
class TestOpenAIReranking:
    ...
    def test_basic_reranking(self, openai_llm, mock_embeddings_response):
        ...
        openai_llm.client.embeddings.create.side_effect = [mock_docs_response, mock_query_response]
        ...
        assert result["ranked_documents"][0] == documents[1]  # Most similar should be first
    ...
    def test_custom_model(self, openai_llm, mock_embeddings_response):
        ...
        with patch.object(OpenAILLM, "RERANKING_MODELS", ["text-embedding-3-small", "text-embedding-3-large"]):
            ...
            assert openai_llm.client.embeddings.create.call_args_list[0][1]["model"] == custom_model
            assert openai_llm.client.embeddings.create.call_args_list[1][1]["model"] == custom_model
