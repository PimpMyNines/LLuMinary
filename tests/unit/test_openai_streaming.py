...
from src.llmhandler.exceptions import ProviderError, ServiceUnavailableError
...
class TestOpenAIStreamGenerate:
    ...
    def test_streaming_with_tool_calls(self, openai_llm):
        ...
        for chunk, stats in openai_llm.stream_generate(
            ...
            messages=[{"message_type": "human", "message": "What's the weather in SF?"}]
        ):
            collected_chunks.append((chunk, stats))
        ...
        assert final_stats["tool_use"]["call_123"]["arguments"] == '{"location": "San Francisco"}'
    ...
    def test_streaming_error_handling(self, openai_llm):
        ...
        openai_llm.client.chat.completions.create.side_effect = Exception("Connection error")
        
        # Call stream_generate and expect an exception
        with pytest.raises(Exception) as exc_info:
            # Consume the generator to trigger the error
            list(openai_llm.stream_generate(
                event_id="test-error",
                system_prompt="You are a test assistant",
                messages=[{"message_type": "human", "message": "Say hello"}]
            ))
    ...
    def test_streaming_image_handling(self, openai_llm):
        ...
        with patch.object(openai_llm, "_count_tokens_from_messages", return_value=100), \
             patch.object(openai_llm, "_format_messages_for_model", return_value=[{"role": "user", "content": "mock formatted content"}]):
            # Create a sequence of chunks
            chunks = [
    ...
    def test_streaming_with_functions(self, openai_llm):
        ...
        with patch.object(openai_llm, "_convert_function_to_tool") as mock_convert, \
             patch.object(openai_llm, "_format_messages_for_model", return_value=[{"role": "user", "content": "Call function"}]):
            ...
            for chunk, stats in openai_llm.stream_generate(
                ...
                messages=[{"message_type": "human", "message": "Call the test function"}],
            ...
            assert openai_llm.client.chat.completions.create.call_args[1]["stream"] is True
    ...
    def test_streaming_token_accumulation(self, openai_llm):
        ...
        for chunk, stats in openai_llm.stream_generate(
            ...
            messages=[{"message_type": "human", "message": "Say something token by token"}]
        ):
            collected_stats.append(stats)
        
        # Verify token counts increase with each chunk
        for i in range(1, len(collected_stats)):
            assert collected_stats[i]["write_tokens"] >= collected_stats[i-1]["write_tokens"]
    ...
    def test_streaming_empty_response(self, openai_llm):
        ...
        for chunk, stats in openai_llm.stream_generate(
            ...
            messages=[{"message_type": "human", "message": "You don't need to respond"}]
        ):
            collected_chunks.append((chunk, stats))
    ...
    def test_streaming_message_conversion(self, openai_llm):
        ...
        with patch.object(openai_llm, "_format_messages_for_model") as mock_format:
            ...
            mock_format.return_value = [{"role": "user", "content": "Converted message"}]
