...
def openai_llm():
    ...
    with patch("src.llmhandler.models.providers.openai.OpenAI") as mock_openai, \
         patch.object(OpenAILLM, "auth") as mock_auth, \
         patch.object(OpenAILLM, "_map_openai_error", return_value=ProviderError(
...
class TestOpenAITimeoutHandling:
    ...
    def test_timeout_retry_behavior(self, openai_llm):
        ...
        with patch("time.sleep") as mock_sleep:
            ...
            result = simple_retry(mock_func, max_retries=3, initial_backoff=0.1, backoff_factor=2.0)
    ...
    def test_timeout_in_embedding_call(self, openai_llm):
        ...
        def embed_with_timeout(should_timeout=True):
            ...
            if should_timeout:
                raise ValueError("Error getting embeddings from OpenAI: Request timed out")
    ...
    def test_custom_timeout_parameter(self):
        ...
        with patch("src.llmhandler.models.providers.openai.OpenAI") as mock_client, \
             patch.object(OpenAILLM, "auth"):
            ...
            assert llm_default.timeout < 120  # Should be a reasonable default under 2 min
    ...
    def test_timeout_recovery_simulation(self, openai_llm):
        ...
        def retry_with_backoff(func, max_retries=5, initial_backoff=1.0, backoff_factor=2.0):
            attempts = 0
            backoff = initial_backoff
        ...
        with patch("time.time", side_effect=simulated_timer), \
             patch("time.sleep", side_effect=simulated_sleep):
...
class TestRawGenerateTimeoutRobustness:
    ...
    def mock_raw_generate(self):
        ...
        def raw_generate_simulation(
            error_sequence=None,
            success_output=("response text", {"read_tokens": 10, "write_tokens": 5, "total_tokens": 15})
        ):
    ...
    def test_format_message_error_handling(self, mock_raw_generate):
        ...
        generate_fn = mock_raw_generate(
            error_sequence=[Exception("Failed to format messages for OpenAI: Formatting error")]
        )
