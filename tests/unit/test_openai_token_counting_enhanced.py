...
class TestImageTokenCounting:
    ...
    @pytest.mark.skip("Image token calculation is implementation-specific and may change")
    def test_calculate_image_tokens_high_detail(self, openai_llm):
        ...
        with patch.object(openai_llm, "MAX_IMAGE_SIZE", 2048), \
             patch.object(openai_llm, "TARGET_SHORT_SIDE", 768), \
             patch.object(openai_llm, "TILE_SIZE", 512), \
             patch.object(openai_llm, "HIGH_DETAIL_TILE_COST", 170), \
             patch.object(openai_llm, "HIGH_DETAIL_BASE_COST", 85):
            ...
            test_cases = [
                ...
                (3000, 2000, 1785),  # Should scale down to 2048x1365, 4x3 tiles (12*170) + base cost (85)
                (300, 300, 255),     # Small image, scales up to 768x768, 2x2 tiles (4*170) + base cost (85)
            ]
            ...
            for width, height, expected_tokens in test_cases:
                ...
                assert tokens == expected_tokens, f"Image {width}x{height} expected {expected_tokens} tokens, got {tokens}"
    ...
    def test_image_token_cost_calculation(self, openai_llm):
        ...
        model_costs = {"read_token": 0.00001, "write_token": 0.00003, "image_cost": None}
        ...
        with patch.object(openai_llm, "get_model_costs", return_value=model_costs):
            ...
            with patch.object(openai_llm, "calculate_image_tokens", return_value=image_tokens):
                ...
                assert abs(cost_breakdown["image_cost"] - expected_image_cost) < 0.000001
...
class TestGenerationCostCalculation:
    ...
    def test_estimate_cost_generation(self, openai_llm):
        ...
        with patch.object(openai_llm, "estimate_tokens", return_value=100):
            ...
            expected_total = breakdown["prompt_cost"] + breakdown["response_cost"] + breakdown["image_cost"]
    ...
    def test_models_pricing_structure(self, openai_llm):
        ...
        for model_name in openai_llm.SUPPORTED_MODELS:
            ...
            assert 0 <= model_costs["read_token"] < 0.001  # Current pricing is under $0.001 per token
    ...
    def test_embedding_pricing_structure(self, openai_llm):
        ...
        for model_name in openai_llm.EMBEDDING_MODELS:
            ...
            assert 0 < cost < 0.001  # Current embedding pricing is under $0.001 per token
...
class TestUsageReportingConsistency:
    ...
    def test_raw_generate_usage_reporting(self, openai_llm):
        ...
        with patch.object(openai_llm, "_call_with_retry", return_value=mock_response), \
             patch.object(openai_llm, "_format_messages_for_model", return_value=[{"role": "user", "content": "Test"}]):
            # Call raw_generate
            ...
            assert abs(usage["total_cost"] - (expected_read_cost + expected_write_cost)) < 0.000001
    
    def test_usage_calculation_fallback(self, openai_llm):
        """Test that usage calculation falls back to defaults when API doesn't provide it."""
        ...
        with patch.object(openai_llm, "_call_with_retry", return_value=mock_response), \
             patch.object(openai_llm, "_format_messages_for_model", return_value=formatted_messages), \
             patch.object(openai_llm, "_count_tokens_from_messages", return_value=10):
...
class TestComprehensiveTokenCounting:
    ...
    def test_special_characters_token_counting(self, openai_llm):
        ...
        special_messages = [
            ...
            {"role": "user", "content": "http://very.long.url.example.com/with/path?and=parameters"} # URL
        ]
