...
class TestImageTokenCounting:
    ...
    def test_calculate_image_tokens_high_detail(self, openai_llm):
        ...
        # Instead of relying on specific token counts, we'll test the relative
        # scaling logic of the algorithm, which is more likely to remain consistent
        # even if OpenAI changes the specific token costs

        # Save original values to restore later
        orig_max_size = openai_llm.MAX_IMAGE_SIZE
        orig_target_short_side = openai_llm.TARGET_SHORT_SIDE
        orig_tile_size = openai_llm.TILE_SIZE
        orig_tile_cost = openai_llm.HIGH_DETAIL_TILE_COST
        orig_base_cost = openai_llm.HIGH_DETAIL_BASE_COST

        try:
            # Set consistent test values for reproducible tests
            openai_llm.MAX_IMAGE_SIZE = 2048
            openai_llm.TARGET_SHORT_SIDE = 768
            openai_llm.TILE_SIZE = 512
            openai_llm.HIGH_DETAIL_TILE_COST = 170
            openai_llm.HIGH_DETAIL_BASE_COST = 85

            # Test case 1: Test scaling logic - larger images should cost more tokens
            base_tokens = openai_llm.calculate_image_tokens(512, 512, "high")
            larger_tokens = openai_llm.calculate_image_tokens(1024, 1024, "high")
            assert (
                larger_tokens > base_tokens
            ), "Larger images should require more tokens"

            # Test case 2: Very large images should be scaled down
            huge_image_tokens = openai_llm.calculate_image_tokens(4000, 4000, "high")
            large_image_tokens = openai_llm.calculate_image_tokens(2048, 2048, "high")
            assert (
                huge_image_tokens == large_image_tokens
            ), "Images larger than MAX_IMAGE_SIZE should be scaled down"

            # Test case 3: Test base cost logic
            small_image_tokens = openai_llm.calculate_image_tokens(256, 256, "high")
            expected_minimum = (
                openai_llm.HIGH_DETAIL_TILE_COST + openai_llm.HIGH_DETAIL_BASE_COST
            )
            assert (
                small_image_tokens >= expected_minimum
            ), "Minimum token cost not applied correctly"

            # Test case 4: Test aspect ratio handling - different aspect ratios should have different token counts
            wide_image_tokens = openai_llm.calculate_image_tokens(1024, 512, "high")
            square_image_tokens = openai_llm.calculate_image_tokens(724, 724, "high")
            assert (
                wide_image_tokens != square_image_tokens
            ), "Images with different aspect ratios should have different token counts"

            # Test case 5: Verify calculation consistency
            test_dimensions = [(800, 600), (1024, 768), (1920, 1080)]
            for w, h in test_dimensions:
                tokens1 = openai_llm.calculate_image_tokens(w, h, "high")
                tokens2 = openai_llm.calculate_image_tokens(w, h, "high")
                assert (
                    tokens1 == tokens2
                ), f"Calculation inconsistent for {w}x{h}: got {tokens1} and {tokens2}"

        finally:
            # Restore original values
            openai_llm.MAX_IMAGE_SIZE = orig_max_size
            openai_llm.TARGET_SHORT_SIDE = orig_target_short_side
            openai_llm.TILE_SIZE = orig_tile_size
            openai_llm.HIGH_DETAIL_TILE_COST = orig_tile_cost
            openai_llm.HIGH_DETAIL_BASE_COST = orig_base_cost
