...
from src.llmhandler.tools.validators import (
    json_serializable,
    validate_params,
...
def test_validate_type_structure_list():
    ...
    assert _validate_type_structure(
        [{"a": 1}, {"b": 2}], 
        List[Dict[str, int]]
    ) is True
...
def test_validate_type_structure_tuple():
    ...
    assert _validate_type_structure((1, "text", True), Tuple[int, str]) is False  # Too long
    ...
    assert _validate_type_structure((1, "2", 3), Tuple[int, ...]) is False  # Mixed types
...
def test_validate_type_structure_dict():
    ...
    assert _validate_type_structure({"a": "1"}, Dict[str, int]) is False  # Value not int
...
def test_validate_type_structure_complex_nested():
    ...
    assert _validate_type_structure(
        {"a": 1, "b": [1, 2, 3], "c": 3},  # [1, 2, 3] not List[str]
        complex_type
    ) is False
